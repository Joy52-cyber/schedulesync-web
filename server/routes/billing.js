...
// GET /api/subscriptions/current - Alias for /subscription
router.get('/current', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      plan: plan,
      status: user.subscription_status || 'active',
      price: plan === 'pro' ? 12 : plan === 'team' ? 20 : 0,
      next_billing_date: isPaid ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : null,
      current_period_end: isPaid ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : null,
      cancel_at: null
    });
...
router.post('/billing-portal', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ url: process.env.FRONTEND_URL || 'https://schedulesync-web-production.up.railway.app' });
  } catch (error) {
